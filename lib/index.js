...
const getChromiumRevision = async (options) => {
    ...
    if (!revision) {
        revision = await Util.resolveBuildId();
...
const revisionHandler = (options) => {
    ...
    if (options.installedBrowser) {
        executablePath = options.installedBrowser.path;
    }
    executablePath = Util.formatPath(executablePath);
    revisionInfo.executablePath = executablePath;
...
const PCR = async (options) => {
    ...
    const localChromium = await detectHandler(options);
    if (!localChromium) {
